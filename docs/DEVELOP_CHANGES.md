...
### Durcissement RLS et révocation des trigger functions

- Fichier : `supabase/migrations/V2__security_fixes.sql`
- Changement : activation de RLS sur `public.admins` (aucune policy = anon/authenticated bloqués) et `REVOKE EXECUTE` sur les fonctions `prevent_unsafe_*` (`student_document`, `student_document_request`, `notification`, `manual_payment_submission`) pour les rôles `PUBLIC`, `anon`, `authenticated`.
- Raison : sans RLS, `admins` était lisible via la clé `anon` (fuite de la liste des emails d’administration). Les fonctions `SECURITY DEFINER` étaient appelables en RPC via `/rest/v1/rpc/<fn>` alors qu’elles ne sont destinées qu’à servir de triggers.
